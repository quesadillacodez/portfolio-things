...
test('all target="_blank" links include rel="noopener noreferrer"', () => {
  ...
  for (const file of files) {
    ...
    const matches = content.match(/<a\s+[^>]*target="_blank"[^>]*>/g) || [];
    for (const match of matches) {
      assert.ok(
        match.includes('rel="noopener noreferrer"') || match.includes('rel="noreferrer noopener"'),
        `Missing rel="noopener noreferrer" in ${file}: ${match}`,
      );
    }
  }
});
